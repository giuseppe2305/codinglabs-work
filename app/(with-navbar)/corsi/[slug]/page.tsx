...
import { Book, Check, CircleAlert, Clock, Library, ShoppingCart, Smile } from "lucide-react";
    ...
    <>
      <Section className="grid grid-cols-1 lg:grid-cols-2 gap-x-16">
        <div className="flex flex-col gap-1">
          ...
          <Section.Subtitle className="max-md:text-center max-md:mt-4">{course.description}</Section.Subtitle>
          ...
          <div className="flex flex-wrap max-sm:flex-col gap-8 items-center mt-6 justify-between">
            ...
            <div className="relative">
              ...
              <span className="text-4xl font-bold text-accent-foreground">€{course.price}</span>
            </div>
          </div>
      ...
      <Section className="grid grid-cols-1 md:grid-cols-[1fr_300px] gap-x-12 border-b-0">
        ...
        <ContentSource source={courseContent} />
        <Card className="p-5 gap-0 max-w-100 max-md:mt-8 w-full mx-auto row-span-2 self-start md:sticky top-24">
          ...
          <CardContent className="mt-4 p-0 flex flex-col gap-3">
            {course.requirements.needed.length > 0 && <h4 className="text-foreground">Obbligatori</h4>}
            ...
            {course.requirements.suggested.length > 0 && <h4 className="text-foreground">Consigliati</h4>}
            {course.requirements.suggested.map((req) => (
              <IconLabel color="text-yellow-400 bg-yellow-400/5" icon={CircleAlert} key={req}>
                {req}
              </IconLabel>
            ))}
            {course.requirements.needed.length === 0 && course.requirements.suggested.length === 0 && (
        ...
        <Section.H3 className="col-span-full mt-12">Contenuto del Corso</Section.H3>
